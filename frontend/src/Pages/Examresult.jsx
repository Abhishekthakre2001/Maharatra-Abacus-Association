...
export default function ExamResult() {
  ...
  const adminId =
    authUser?.id || authUser?.admin_id || localStorage.getItem("admin_id");
  const {
    page,
    limit,
    search,
  ...
  const { data: response, loading } = useFetchData(
    () => {
      if (!adminId) return Promise.resolve(null);

      return ExamResultApi.getExamResultsByAdmin(
        adminId,
        page,
        limit,
        debouncedSearch,
  ...
  const tableData = useMemo(() => {
    return (submittedResults || []).map((item) => {
  ...
  const handleExport = () => {
    downloadExcelFile(() => ExamResultApi.exportexportExamResultsByAdmin(adminId), "exam-result.xlsx");
  };
  return (
    <>
      {/* <Sidebar
